...
class GroupChatPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<GroupChatPageController>(
        init: GroupChatPageController(),
        builder: (_) => Scaffold(
          appBar: FiveCMAppbar(
            context,
            title: "${_.group!.groupName}",
            right: assetImage('ic_more', width: 28.h)
                .intoContainer(
                padding: EdgeInsets.symmetric(horizontal: 22.w),
                height: double.infinity)
                .intoGesture(onTap: (){
              Get.to(()=>QRPage(_.group!.gid));
            }),
            back: true,
          ),
          body: Column(
                ...
                child: GetBuilder<AppController>(
                  ...
                  builder: (i) => SmartRefresher(
                    ...
                    child: ListView.builder(
                        ...
                        itemCount: i.groupMessageMap[_.group!.gid]!.length,
                        itemBuilder: (BuildContext context, int index) {
                          return AutoScrollTag(
                              key: ValueKey(index),
                              controller: _.autoScrollController,
                              index: index,
                              child: ChatMsgView(i.groupMessageMap[
                              _.group!.gid]![index],true)
                                  .intoContainer(
...
class GroupChatPageController extends GetxController {
  ...
  Future<void> getMoreMessage() async {
    var l = appController.groupMessageMap[group!.gid];
    var perMsgResponse = await dio.get(
        "/getMorePersonalMessage?fromId=${user.uid}&toId=${group!.gid}&timestamp=${l!.length > 0 ? l.last.msg.timestamp : DateTime.now().millisecondsSinceEpoch}");
    ...
    appController.groupMessageMap[group!.gid]!.addAll(arr0);
    update();
  }
}
...
class ChatInputView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return GetBuilder<GroupChatInputViewController>(
        init: GroupChatInputViewController(),
        builder: (_) {
          return Column(
            children: [
              Container(
...
class GroupChatInputViewController extends GetxController {
  Group? group = Get.find(tag: "currentGroup");
  ...
  Future<void> sendImage(url) async {
    ...
    var message = Message(
        ...
        PGMessage(
            ...
            isGroup: false));
    appController.addGroupMessage(group!.gid!, message);
  ...
  void send() async {
    ...
    if (result["success"] == true) {
      ...
      // var list=appController.groupMessageMap[contact!.account];
      autoScrollController.scrollToIndex(0,
          preferPosition: AutoScrollPosition.begin);
    }
  }
